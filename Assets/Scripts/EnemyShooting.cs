...
public class EnemyShooting : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // Debug.Log(distance);
        if(transform.position.y<-1.2f && transform.position.y>-3.5f && !isDectected && !compareColors(GetComponent<SpriteRenderer>().color, player.GetComponent<SpriteRenderer>().color)){
            Shoot();
            isDectected = true;
        }
